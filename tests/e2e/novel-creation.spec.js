...
test.describe('Novel Creation E2E', () => {
  ...
  test('should create a new novel through the UI', async () => {
    ...
    await expect(page.locator('.modal-dialog h2:has-text("Create a New Novel")')).toBeVisible();
  ...
  test('should validate novel name and show error for invalid names', async () => {
    ...
    if (!isDisabled) {
      ...
      const errorMessage = page.locator('.modal-dialog .error-message');
  ...
  test('should allow canceling novel creation', async () => {
    ...
    // Step 3: Click Cancel
    const cancelButton = page.locator('.modal-dialog button:has-text("Cancel")');
  ...
  test('should show loading state during creation', async () => {
    ...
    await expect(page.locator('.modal-dialog')).not.toBeVisible({ timeout: 10000 });
  });
});
