...
function ensureGitRepo(novelPath) {
  ...
  try {
    ...
    execFileSync('git', ['config', 'user.name', 'Netwriter'], { cwd: novelPath, stdio: 'ignore' });
    execFileSync('git', ['config', 'user.email', 'netwriter@localhost'], { cwd: novelPath, stdio: 'ignore' });
    return null;
  } catch (err) {
